...
@section('content')
    ...
    @else
        <div class="grid grid-cols-2 gap-3">
            @foreach ($rows as $row)
                <a href="{{ route('ec-board.centers', $row['barangay']) }}" class="card-modern p-4 hover:shadow-md transition-shadow flex items-center justify-between">
                    <div class="flex items-center gap-2.5">
                        <div class="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
                            <i class="ti ti-map-pin text-brand" style="font-size: 16px;" aria-hidden="true"></i>
                        </div>
                        <div>
                            <p class="font-bold text-sm text-gray-800">{{ $row['barangay']->name }}</p>
